...
module.exports = function() {
  ...
  api.getDescription = function() {
    return "A custom algorithm that is based on expected win";
  };
  /*
   * 100 points per game
  ...
  api.runStats = function(result,players,done) {
    ...
    if(result.technical) {
      result.winner.forEach(function(winner) {
        ...
        current.currentRating += current.rank + 10;
      ...
      var margin = (result.loserLeft*10/7);
      winningRating = getWinnerRating(winnerRank,loserRank,margin);
      
      loserRating = getLoserRating(winnerRank,loserRank,margin);
